...
echo '6.'
cat data-hold/legislators-current.json |   jq '.[] .bio .religion' | sort | uniq -c | sort -r | tr -d '"'
...
echo '8.'
cat data-hold/legislators-current.json |   jq --raw-output '.[] .terms[0] .start'| sort| grep -oE '[[:digit:]]{4}' |  head -n 1
...
echo '10.'
cat data-hold/legislators-current.json |   jq --raw-output '.[] .terms[-1:][0].party' | grep -v 'Republican'| grep -v 'Democrat'| wc -l
...
echo '17.'
cat data-hold/congress-twitter-profiles.json |   jq '.[] .followers_count'| sort| sort -nr | head -n 1
